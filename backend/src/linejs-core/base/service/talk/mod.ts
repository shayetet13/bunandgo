...
export class TalkService implements BaseService {
	...
	readonly #e2eeTargets = new Set<string>();
	readonly #checkedE2eeTargets = new Set<string>();
	...
	sendCompactMessage(options: SendCompactMessageOptions): Promise<CompactMessageResponse> {
		...
		return this.sendCompactPlainMessage({
			...
			if (options.e2ee === undefined && (code === 82 || code === 99)) {
				...
				// the logged-in client so subsequent sends avoid a failed /CA5 RTT.
				this.#rememberE2eeTarget(options.to);
	...
	async #sendCompactMessageCold(options: SendCompactMessageOptions): Promise<CompactMessageResponse> {
		await this.#requiresE2ee(options.to);
		return this.sendCompactMessage(options);
	}

	async #requiresE2ee(to: string): Promise<boolean> {
	...
	#rememberE2eeTarget(to: string): void {
		if (this.#e2eeTargets.has(to)) return;
		this.#e2eeTargets.add(to);
		this.#checkedE2eeTargets.add(to);
		void this.client.storage.set(COMPACT_E2EE_TARGET_KEY_PREFIX + to, true).catch(() => {});
	}
	...
	/** Builds the real compact request locally so the first reply is already JIT-hot. */
	async prewarmCompactSendTarget(to: string): Promise<void> {
		...
		if (requiresE2ee) {
			...
			packCompactPlainMessage(seqId, to, "warm");
		}
