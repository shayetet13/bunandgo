...
/**
 * Runs a login RPC, re-issuing it when the connection breaks under it.
 *
...
async function withTransportRetry<T>(client: BaseClient, label: string, call: () => Promise<T>): Promise<T> {
	for (let attempt = 0; ; attempt++) {
		try {
			...
			if (attempt >= MAX_TRANSPORT_RETRIES || !isTransientTransportFailure(message)) {
				throw error;
			}
...
export class Login {
	...
	async login(options?: LoginOption) {
		if (!options) {
			...
			await this.withQrCode({ v3: (options as QrCodeLoginOption).v3 });
			...
			await this.withPassword(options as PasswordLoginOption);
		} else {
			await this.withQrCode();
		}
		await this.ready();
	...
	public async requestSQR(): Promise<string> {
		...
		let { 1: url } = await withTransportRetry(this.client, "createQrCode", () => this.createQrCode(sqr));
		...
		if (await this.checkQrCodeVerified(sqr)) {
			...
			// See requestSQR2: the scan already succeeded at this point.
			const response = await withTransportRetry(this.client, "qrCodeLogin", () => this.qrCodeLogin(sqr));
	...
	/**
	 * Waits for the user to act on a QR or PIN, by long-polling LINE.
	 *
	 * The wait is bounded by wall clock — `maxCount` intervals' worth, which
	 * is what LINE advertises the code is good for — rather than by a count
	 * of attempts. That distinction is the point: a poll whose connection
	 * broke never actually asked the question, so re-issuing it consumes the
	 * remaining time but not one of the answers we were promised. Counting it
	 * as a spent attempt is what let a single routine GOAWAY end the login
	 * and replace a QR the user was mid-scan on.
	 *
	 * Returns false once the budget is gone, which is the caller's cue that
	 * the code really has expired and a fresh one is needed.
	 */
	private async pollUntilVerified(
		methodName: "checkQrCodeVerified" | "checkPinCodeVerified",
		qrcode: string,
		maxCount: number,
		intervalSec: number,
	): Promise<boolean> {
		const intervalMs = intervalSec * 1000;
		const deadline = Date.now() + intervalMs * maxCount;
		let transportFailures = 0;

		while (Date.now() < deadline) {
			const holdMs = Math.min(intervalMs, deadline - Date.now());
			...
			try {
				...
				if (isTransientTransportFailure(message)) {
					// The socket died, not the session. Ask again — the code
					// on screen stays valid, so a scan in flight still lands.
				...
				// A poll that answered instantly would otherwise let this loop
				// spin at full speed until the deadline.
				const elapsed = Date.now() - startedAt;
				if (elapsed < MIN_POLL_SPACING_MS) await sleep(MIN_POLL_SPACING_MS - elapsed);
			}
		}
		return false;
	}
