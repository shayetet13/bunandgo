import type { BaseClient } from "../core/mod.ts";
import type { Operation, SquareEvent } from "@evex/linejs-types";

export interface SyncData {
	square?: string;
	talk: {
		revision?: number | bigint;
		globalRev?: number | bigint;
		individualRev?: number | bigint;
	};
}

function sleep(time: number) {
	return new Promise<void>((resolve) => {
		setTimeout(resolve, time);
	});
}

function pushErrorData(error: unknown): Record<string, unknown> {
	if (error instanceof Error) {
		return {
			name: error.name,
			message: error.message,
			cause: error.cause instanceof Error ? { name: error.cause.name, message: error.cause.message } : error.cause,
		};
	}
	return { message: String(error) };
}

export class Polling {
	sync: SyncData = { talk: { revision: 0, globalRev: 0, individualRev: 0 } };
	client: BaseClient;
	islisten: boolean = false;
	listenTarget: number[] = [3, 8];
	/**
	 * Set once the session owning this poller is torn down. See `stop()`.
	 */
	stopped: boolean = false;

	constructor(client: BaseClient) {
		this.client = client;
	}

	/**
	 * Ends the pusher loop and drops the connection it is holding.
	 *
	 * `initLegyPusher` used to loop on `client.authToken` alone, and nothing
	 * in the codebase ever clears that field — so its only exit condition was
	 * unreachable. Every `stopBot()` and every watchdog reconnect therefore
	 * abandoned a *live* pusher that went on re-signing-on to LINE with the
	 * credential of a session that no longer existed.
	 *
	 * That is not merely a leak that logs noise. Concurrent sign-ons for one
	 * account are exactly the multi-device conflict LINE answers with
	 * NOT_AUTHORIZED_DEVICE / V3_TOKEN_CLIENT_LOGGED_OUT, and each such logout
	 * made the watchdog reconnect — abandoning another pusher. The leak kept
	 * re-causing the very logout it was reporting, which is why the symptom
	 * grew worse the longer a process ran and why restarting it (or moving
	 * hosts) only ever helped until enough reconnects had accumulated again.
	 */
	stop(): void {
		this.stopped = true;
		const conn = this.client.push.conns[0];
		if (!conn) return;
		// Fire-and-forget: callers tear down synchronously, and a close that
		// fails because the connection is already dead reached the goal anyway.
		void conn.close().catch(() => {});
	}

	/**
	 * Listens to square events and yields them as they are received.
	 *
	 * @param options - Configuration options for listening to square events.
	 * @param options.signal - An AbortSignal to cancel the polling.
	 * @param options.onError - A callback function to handle errors.
	 * @param options.pollingInterval - The interval in milliseconds between polling requests. Defaults to 1000ms.
	 *
	 * @yields {SquareEvent} - The events received from the square.
	 *
	 * @deprecated
	 */
	async *_listenSquareEvents(
		options: {
			signal?: AbortSignal;
			onError?: (error: unknown) => void;
			pollingInterval?: number;
		} = {},
	): AsyncGenerator<SquareEvent, void, unknown> {
		const { signal, onError, pollingInterval } = {
			pollingInterval: 1000,
			...options,
		};
		let continuationToken: string | undefined;
		while (true) {
			try {
				const response = await this.client.square.fetchMyEvents({
					syncToken: this.sync.square,
					continuationToken,
					limit: 100,
				});
				this.sync.square = response.syncToken;
				continuationToken = response.continuationToken;
				for (const event of response.events) {
					yield event;
				}
			} catch (error) {
				if (onError) {
					onError(error);
				}
			}
			await sleep(pollingInterval);
			if (signal?.aborted) {
				break;
			}
		}
	}

	/**
	 * Listens for talk events by polling the server at a specified interval.
	 *
	 * @param {Object} [options] - Configuration options for the polling.
	 * @param {AbortSignal} [options.signal] - An AbortSignal to cancel the polling.
	 * @param {(error: unknown) => void} [options.onError] - A callback function to handle errors.
	 * @param {number} [options.pollingInterval=1000] - The interval in milliseconds between each poll.
	 *
	 * @yields {Operation} - Yields each operation event received from the server.
	 *
	 * @returns {AsyncGenerator<Operation, void, unknown>} - An async generator that yields operation events.
	 *
	 * @deprecated
	 */
	async *_listenTalkEvents(
		options: {
			signal?: AbortSignal;
			onError?: (error: unknown) => void;
			pollingInterval?: number;
		} = {},
	): AsyncGenerator<Operation, void, unknown> {
		const { signal, onError, pollingInterval } = {
			pollingInterval: 100,
			...options,
		};
		while (true) {
			try {
				const response = await this.client.talk.sync({
					...this.sync.talk,
					limit: 100,
				});
				if (response.fullSyncResponse && response.fullSyncResponse.nextRevision) {
					this.sync.talk.revision = response.fullSyncResponse.nextRevision;
				}
				if (response.operationResponse && response.operationResponse.globalEvents && response.operationResponse.globalEvents.lastRevision) {
					this.sync.talk.globalRev = response.operationResponse.globalEvents.lastRevision;
				}
				if (
					response.operationResponse &&
					response.operationResponse.individualEvents &&
					response.operationResponse.individualEvents.lastRevision
				) {
					this.sync.talk.individualRev = response.operationResponse.individualEvents.lastRevision;
				}
				if (!(response.operationResponse && response.operationResponse.operations)) {
					continue;
				}
				for (const event of response.operationResponse.operations) {
					this.sync.talk.revision = event.revision;
					yield event;
				}
			} catch (error) {
				if (onError) {
					onError(error);
				}
			}
			await sleep(pollingInterval);
			if (signal?.aborted) {
				break;
			}
		}
	}

	async initLegyPusher(cb?: () => void) {
		if (this.islisten) {
			cb && cb();
			return;
		}
		this.islisten = true;
		let consecutiveFailures = 0;
		try {
			while (this.client.authToken && !this.stopped) {
				let connected = false;
				try {
					await this.client.push.initializeConn(1, this.listenTarget);
					connected = true;
					cb && cb();
					await this.client.push.InitAndRead(this.listenTarget);
					consecutiveFailures = 0;
				} catch (error) {
					consecutiveFailures++;
					this.client.log(connected ? "LegyPusherError" : "LegyPusherError_cannot_init", {
						error: pushErrorData(error),
						attempt: consecutiveFailures,
					});
				} finally {
					// A failed stream must not remain as conns[0]; otherwise the next
					// iteration writes into an already aborted request body.
					const stale = this.client.push.conns[0];
					if (stale) {
						await stale.close().catch(() => {});
						const index = this.client.push.conns.indexOf(stale);
						if (index >= 0) this.client.push.conns.splice(index, 1);
					}
				}

				// Checked again after the await above: a teardown that landed
				// mid-flight must not be followed by another reconnect.
				if (!this.client.authToken || this.stopped) break;
				// Recover quickly from a one-off reset, but back off persistent
				// failures so a broken route cannot spin or hammer LINE.
				const retryMs = Math.min(4_000, 250 * 2 ** Math.min(consecutiveFailures, 4));
				await sleep(retryMs);
			}
		} finally {
			this.islisten = false;
		}
	}

	listenSquareEvents(): ReadableStream<SquareEvent> {
		this.client.push.sqStream.renew();
		this.initLegyPusher();
		return this.client.push.sqStream.stream;
	}

	listenTalkEvents(): ReadableStream<Operation> {
		this.client.push.opStream.renew();
		this.initLegyPusher();
		return this.client.push.opStream.stream;
	}
}
