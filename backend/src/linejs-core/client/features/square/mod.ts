import type { Square as SquareRaw, SquareChat as SquareChatRaw } from "@evex/linejs-types";
...
export class SquareChat extends TypedEventEmitter<SquareChatEvents> {
	...
	async sendVideo(data: Blob, filename = "video", durationMs?: number): Promise<{ objId: string; objHash: string }> {
	 ...
	 */
	async sendAudio(data: Blob, filename = "voice.m4a", durationMs?: number): Promise<{ objId: string; objHash: string }> {
		return await this.#sendMedia("audio", data, filename, durationMs);
	}

	async #sendMedia(type: ObjType, data: Blob, filename: string, durationMs?: number): Promise<{ objId: string; objHash: string }> {
		...
		while (!param.signal?.aborted && this.#client.base.authToken) {
			try {
				...
				for (const event of response.events) {
					...
					} else if (event.type === "NOTIFIED_LEAVE_SQUARE_CHAT" && event.payload.notifiedLeaveSquareChat) {
						this.emit("leave", event.payload.notifiedLeaveSquareChat);
					} else if (event.type === "NOTIFIED_JOIN_SQUARE_CHAT" && event.payload.notifiedJoinSquareChat) {
						this.emit("join", event.payload.notifiedJoinSquareChat);
					} else if (event.type === "NOTIFIED_DESTROY_MESSAGE" && event.payload.notifiedDestroyMessage) {
