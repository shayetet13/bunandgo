...
import { retryTalkMessageOperation } from "./talk-message-retry.ts";
...
// Note: voice-call feature (client/features/call/*) intentionally not
// vendored — it needs opusscript/node:dgram and is out of scope for a
// text-reply bot. Call-related events/getters below are dropped too.
	...
	listen(opts: ListenOptions = { talk: true, square: true }): void {
		...
		if (opts.talk) {
			(async () => {
				for await (const event of polling.listenTalkEvents()) {
					...
					if (event.type === "SEND_MESSAGE" || event.type === "RECEIVE_MESSAGE") {
						try {
							...
							// Plain messages need no async work. Encrypted messages keep
							// this exact operation in memory while a routine gf/enc
							// GOAWAY is retried; the old uncaught await killed the whole
							...
							Object.defineProperties(message, {
								...
								[Symbol.for("linebot.decryptMs")]: { value: decryptMs },
								[Symbol.for("linebot.receiveSource")]: { value: "push" },
							});
							this.emit("message", message);
						} catch (error) {
							// One corrupt/permanently undecryptable message must be visible,
							// but it must never terminate receipt of every later message.
							this.base.log("TalkMessageError", {
				...
				// Same hazard as the push keepalive: an uncaught rejection in
				// this detached loop kills the process rather than just this
				// stream. Reported under its own type because a dead listener
				// is invisible to an auth-only health check — the session stays
				// valid while the bot quietly stops hearing anything.
				this.base.log("ListenerStopped", {
					stream: "talk",
				...
				for await (const event of polling.listenSquareEvents()) {
					...
					if (event.type === "NOTIFICATION_MESSAGE") {
						...
						Object.defineProperties(message, {
							...
							// Square's ordinary listener is backed by the account-wide
							// fetchMyEvents chain. Keep it distinct from the per-room
							// dedicated poller and from Talk's actual push stream.
							[Symbol.for("linebot.receiveSource")]: { value: "normal-poll" },
						});
						this.emit("square:message", message);
					}
				}
			})().catch((err) => {
				this.base.log("ListenerStopped", {
