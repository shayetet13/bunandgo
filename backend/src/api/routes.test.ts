...
describe("PATCH /api/users/:id refuses bot-affecting changes for a user outside this worker's scope", () => {
	...
	test("botQuota: refuses to lower a quota that would stop bots outside scope, and does not persist the new quota", async () => {
		cookie = adminCookie;
		const created = await request("/api/users", {
			method: "POST",
		...
		await request("/api/bots", { method: "POST", body: JSON.stringify({ name: "dana 2" }) });

		cookie = adminCookie;
		process.env.WORKER_OWNER_EXCLUDE = String(dana.id);
		const res = await request(`/api/users/${dana.id}`, {
			method: "PATCH",
			body: JSON.stringify({ botQuota: 1 }),
		});
	...
	test("active=false: refuses rather than silently skipping the stop for a bot outside scope", async () => {
		...
		cookie = adminCookie;
		process.env.WORKER_OWNER_EXCLUDE = String(erin.id);
		const res = await request(`/api/users/${erin.id}`, {
			method: "PATCH",
			body: JSON.stringify({ active: false }),
		});
		expect(res.status).toBe(409);
	...
	test("DELETE: refuses rather than orphaning bots owned by another worker", async () => {
		cookie = adminCookie;
		const created = await request("/api/users", {
			method: "POST",
		...
		expect((await request(`/api/users/${frank.id}`, { method: "DELETE" })).status).toBe(409);

		delete process.env.WORKER_OWNER_EXCLUDE;
		expect((await request(`/api/bots/${bot.id}/rules`)).status).toBe(200);
		expect((await request(`/api/users/${frank.id}`, { method: "DELETE" })).status).toBe(200);
		expect((await request(`/api/bots/${bot.id}/rules`)).status).toBe(404);
	});
});
