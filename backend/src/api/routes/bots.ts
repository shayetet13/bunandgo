...
import { z } from "zod";
import { BotAccessError, createBot, listBotsForUser, oldestBotOwnedBy, reorderBotsForUser } from "../../bot/bots.ts";
...
botsRoute.get("/", (c) => c.json(listBotsForUser(requestUser(c)!)));
...
botsRoute.put("/order", async (c) => {
	...
	try {
		...
		if (error instanceof BotAccessError) return c.json({ error: error.message }, 403);
...
botsRoute.post("/", async (c) => {
	...
	if (user.role !== "admin" && listBotsForUser(user).length >= user.botQuota) {
		return c.json(
			{
