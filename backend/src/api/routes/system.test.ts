...
describe("/api/system/square-poll-quiet", () => {
	...
	test("GET defaults to 15ms, PUT rejects out-of-range and non-integer values", async () => {
		const app = buildApp();
		const adminCookie = `${SESSION_COOKIE}=${createSession()}`;
		expect(await (await quietRequest(app, adminCookie)).json()).toEqual({ quietMs: 15 });
