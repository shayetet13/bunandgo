...
function visibleBotIds(c: Parameters<typeof requestUser>[0]): number[] | undefined {
	...
	return user.role === "admin" ? undefined : listBotIdsForUser(user);
}

export function allRecentLatency(limit: number): LatencySample[] {
	return latencyTracker.recent(limit);
}

function allRecentFastPath(limit: number): FastPathSample[] {
	return fastPathTracker.recent(limit);
}
...
metricsRoute.get("/lane-race", (c) => {
	...
	// Tagged with workerId so a lane's React key stays stable across the
	// per-process history the dashboard groups by.
