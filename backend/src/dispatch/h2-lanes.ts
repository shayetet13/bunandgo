...
import {
	...
	predictSendCompletionFromStats,
...
interface Lane {
	...
	sendRttMs?: number;
	/** Write-time p95 for the shared lane prior; avoids sorting on selection. */
	sendP95Ms?: number;
	pollRttMs?: number;
...
export function sendLaneScore(
	...
	const routeProfile = freshSendRouteProfile(lane.sendRouteProfiles, routeKey, now);
	const prediction = routeProfile
		? predictSendRouteCompletion(routeProfile, lane.inFlight, lane.streamCapacity)
		: lane.sendP95Ms !== undefined
			? predictSendCompletionFromStats(lane.sendRttMs, lane.sendP95Ms, lane.inFlight, lane.streamCapacity)
			: predictSendCompletion(lane.sendRttSamples, lane.sendRttMs, lane.inFlight, lane.streamCapacity);
...
function recordApplicationRtt(lane: Lane, role: LaneRole, sampleMs: number, routeKey?: string): void {
	...
	const sendSummary = summarizeSendSamples(lane.sendRttSamples)!;
	lane.sendRttMs = sendSummary.p50Ms;
	lane.sendP95Ms = sendSummary.p95Ms;
