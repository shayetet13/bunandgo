...
function envDefaultQuietMs(): number {
	const raw = Number(process.env.SQUARE_FAST_POLL_QUIET_MS ?? QUIET_MIN_MS);
	return Number.isFinite(raw) ? clamp(Math.round(raw), QUIET_MIN_MS, QUIET_MAX_MS) : QUIET_MIN_MS;
}
