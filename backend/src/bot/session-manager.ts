...
/**
 * Dashboard, SQLite write-behind, metrics persistence, and logs are observers
 * of a reply — never prerequisites for dispatching or resuming the poll loop.
 * EventEmitter listeners execute synchronously, so keeping this as a macrotask
 * boundary prevents WebSocket JSON serialization from stealing the current
 * receive/send turn.
 */
function deferReplyObserver(work: () => void): void {
	setImmediate(() => {
		try {
			work();
		} catch (err) {
			console.error("deferred reply observer failed:", err);
		}
	});
}
	...
	try {
		...
		if (lineMid) {
			...
			if (idLockOutcome === "first_login") {
				...
				// the sweep could not reach (e.g. it was mid-session at the time).
...
async function handleIncoming(
	...
	if (prewarmGuardBotId === undefined) {
		...
		const messageIn = { botId, surface, text, targetMid, ts, createdTime: lineCreatedTime, fromMid: senderMid };
		// The reply promise has already started above. `emit` can synchronously
		// JSON.stringify + ws.send for every dashboard, so defer both observers
		// until after this receive/send turn has yielded to the event loop.
		deferReplyObserver(() => {
			botEvents.emit("message_in", messageIn);
			// Durable copy for the live feed. The write-behind worker means this
			// is not a disk write on the bot loop, and the defer keeps its queue
			// bookkeeping out of the reply dispatch turn as well.
			enqueueMessageIn(messageIn);
		});
	}
	await replyPromise;
}
...
function startScheduledPostRunner(): void {
	for (const post of listAllPendingScheduledPosts()) armScheduledPostTimer(post);
}
...
async function sendTimed(
	...
	// The upstream reply is now settled. Keep the poll chain free to fetch the
	// next event, then persist/display the measurements on a later turn.
	deferReplyObserver(() => {
