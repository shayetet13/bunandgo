...
export function runMigrations(db: Database): string[] {
	// One immediate transaction serializes schema work across processes that
	// boot together against the same SQLite file. The old per-migration
	// transactions let two processes observe a migration as missing, then race
	// the same ALTER/INSERT. busy_timeout on the connection makes the second
	// process wait here and re-read the applied set afterward.
