...
export const api = {
	...
	createAnnouncement: (input: { title: string; body: string; isModalAlert: boolean; isPinned: boolean }) =>
		request<Announcement>("/api/announcements", { method: "POST", body: JSON.stringify(input) }),
	updateAnnouncement: (id: number, input: { title: string; body: string; isModalAlert: boolean; isPinned: boolean }) =>
