...
export function summarizeServer(server?: ServerStatus): ServerSummary {
	...
	if (!server.reachable)
		return { label: "ติดต่อไม่ได้", detail: server.detail ?? "ไม่สามารถเชื่อมต่อเครื่องนี้ผ่านเครือข่ายส่วนตัว", tone: "bad" };
	...
	if (server.load.exceeded)
		return {
			label: `โหลด ${capacity}%`,
			detail: `โหลดเกินขีดจำกัด · CPU ${server.load.cpuPercent.toFixed(1)}% · RAM ${server.load.memoryPercent.toFixed(1)}%`,
			tone: "bad",
		};
	if (capacity >= 80)
		return {
			label: `โหลด ${capacity}%`,
			detail: `โหลดใกล้ขีดจำกัด · CPU ${server.load.cpuPercent.toFixed(1)}% · RAM ${server.load.memoryPercent.toFixed(1)}%`,
			tone: "warn",
		};
	return {
		label: `โหลด ${capacity}%`,
		detail: `ปกติ · CPU ${server.load.cpuPercent.toFixed(1)}% · RAM ${server.load.memoryPercent.toFixed(1)}%`,
		tone: "go",
	};
}
