...
describe("network speed panel RTT tone", () => {
	test("shows the fastest application-measured lane as go", () => {
		expect(rttToneClass(true, true)).toBe("chip--go");
	});

	test("shows a slower measured standby as neutral, not failed", () => {
		expect(rttToneClass(true, false)).toBe("chip--idle");
	});
