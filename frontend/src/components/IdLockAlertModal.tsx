			...
			<div className="panel id-lock-modal" onClick={(e) => e.stopPropagation()}>
				...
				{isNameMismatch ? (
					<p className="id-lock-modal-line">
						...
						{event.attemptedName ? <> (<strong>{event.attemptedName}</strong>)</> : null} ไม่ตรงกับชื่อที่ผูกไว้ตั้งแต่ครั้งแรก
						{event.previousName ? <> (<strong>{event.previousName}</strong>)</> : null} แม้บัญชีจะตรงกัน ระบบจึงปฏิเสธการเข้าสู่ระบบนี้โดยอัตโนมัติ
						<br />
