		...
		<section className="panel" style={{ padding: "var(--space-md)" }}>
			...
			<div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: "var(--space-md)" }}>
				{race.lanes.length === 0 ? (
					...
					return (
						<div key={`${lane.workerId}-${lane.origin}-${lane.laneId}`} style={{ display: "grid", gridTemplateColumns: "minmax(110px, 1fr) auto auto", gap: "var(--space-sm)", alignItems: "center", padding: "0.55rem 0.65rem", border: "1px solid var(--border-hair)", borderRadius: "var(--radius-sm)" }}>
							<div style={{ minWidth: 0 }}>
								...
								<div className="hint" style={{ fontSize: "0.72rem" }}>{shortOrigin(lane.origin)} · {lane.workerId} · {lane.state} · {rtt?.toFixed(1) ?? "—"} ms</div>
							</div>
