			...
			<div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: "var(--space-md)" }}>
				{race.lanes.length === 0 ? (
					...
					race.lanes.map((lane, index) => {
						...
						return (
							<div
								key={`${lane.workerId}-${lane.origin}-${lane.laneId}`}
								style={{
									display: "grid",
			...
			<div style={{ marginTop: "var(--space-md)", paddingTop: "var(--space-md)", borderTop: "1px solid var(--border-hair)" }}>
				<div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: "var(--space-sm)", flexWrap: "wrap" }}>
					...
					<span className="hint" style={{ fontSize: "var(--text-xs)" }}>
						เรียงเหตุการณ์ล่าสุด · เก็บย้อนหลัง {race.retentionDays || 30} วัน
					</span>
				</div>
				<div
					style={{
				...
				>
					...
					{history.map((item, index) => {
						if (item.kind === "lane") {
							...
							return (
								<div
									key={`lane-${event.ts}-${event.laneId}-${event.role}-${index}`}
									style={{
										display: "flex",
										alignItems: "center",
										flexWrap: "wrap",
						...
						return (
							<div
								key={`latency-${sample.ts}-${sample.botId}-${index}`}
								style={{
									display: "flex",
									alignItems: "center",
									flexWrap: "wrap",
