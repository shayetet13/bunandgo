...
| Database | **`bun:sqlite`** (built-in) WAL mode | ไม่มี ORM — query ตรงๆ, prepared statements, เขียนพร้อมกันจากหลาย thread ได้ด้วย `busy_timeout` (บอทหลักกับ sqlite-writer thread) |
...
| Server 2 (บอทจริง) | Linode/Akamai VM | รัน `linebot-worker` process เดียว (ไม่แบ่ง shard ไม่ pin CPU) |
...
ส่วนใหญ่มาจากฟีเจอร์ที่มีเหตุผล/incident รองรับชัดเจน (การ
เลือก SEND lane แบบพยากรณ์ผล, security hardening, ระบบประกาศแอดมิน — ดูหัวข้อ 7.4) ไม่ใช่การ
...
### 3.1 Process เดียว ไม่แบ่ง shard

Server 2 รัน `linebot-worker` process เดียวดูแลบอททุกตัวของทุก owner ไม่มีการแบ่งตาม owner
ไม่มี control plane / shard และไม่ pin CPU — ให้ OS scheduler เลือก core เอง

เดิมเคยแบ่ง owner ไปหลาย process (`WORKER_OWNER_SCOPE`/`EXCLUDE`, `balanced-sticky`,
`worker-topology.json`, proxy คำสั่ง + relay event ข้าม process) เพื่อใช้หลาย core แล้วเอาออกทั้งหมด
ข้อที่ยังต้องรู้:

- reply/poll hot path เป็น **single JS thread เดียว** จำนวนบอทที่รันพร้อมกันจึงถูกจำกัดด้วย
  ความเร็วของ 1 core (fast-poll แบบ zero-delay ก็กิน core นั้นเกือบเต็มตลอดเวลา) — ดูจำนวน
  fast-poll slot (`SQUARE_FAST_POLL_SLOTS`) และจำนวน lane ที่ตั้งใน env
- state ในหน่วยความจำที่ใช้ร่วมกันระหว่างบอทของ owner เดียวกัน (`roomAnswers` กันตอบซ้ำ,
  primary/secondary handoff, sibling fast-poll nudge) อยู่ใน process เดียวกันเสมอ
- ตาราง `owner_worker_assignments` ยังอยู่ในฐานข้อมูลแต่ไม่มีโค้ดอ่านหรือเขียนแล้ว
...
- **Server 2** (`deploy-server2.sh`):
  ...
  7. สลับ symlink (blue-green) → restart service → health check → **rollback อัตโนมัติ**ถ้า
...
SQUARE_FAST_POLL_ALLOW_ZERO_MS=0          # ต้องตั้ง =1 คู่กับ INTERVAL_MS=0 (ดู worker.env.example)
...
# ===== Worker label =====
WORKER_ID=                    # ป้ายที่ประทับบน lane_race_events (default "standalone"); prod ตั้ง primary
...
2. **อย่าเพิ่มชั้นแบ่งงานข้าม process ก่อนมีหลักฐานว่าจำเป็น** — ระบบ shard (แบ่ง owner ไปหลาย
   process พร้อม proxy/relay/validation ตอน boot) ต้องใช้โค้ดกว่า 1,700 บรรทัด (รวมเทสต์) + config หลายไฟล์
   ที่ต้องตรงกันเป๊ะ และเคยทำ deploy ปกติใช้งานไม่ได้เพราะ env ตั้งไม่ครบ ตอนนี้เอาออกแล้ว
   กลับเป็น process เดียว ถ้าจะแบ่งอีกต้องวัดก่อนว่าคอขวดคือ CPU จริง ไม่ใช่ lane/slot
...
| `deploy/server2/README.md` | รายละเอียด service/env setup บน Server 2 |
