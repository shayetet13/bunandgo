...
>
> อัปเดต 21 สิงหาคม 2026 — ปิดช่องโหว่ "ชื่อว่างบายพาส": เดิมถ้า LINE ส่ง displayName ว่างกลับมาตอน login ระบบจะข้ามการเช็คชื่อไปเลย (ตราบใด mid ยังตรง) ตอนนี้ชื่อว่างที่ส่งเข้ามาถือเป็น `name_mismatch` เสมอเมื่อบอทเคยบันทึกชื่อไว้แล้ว — ดูหัวข้อ 2
...
การเช็คชื่อจะ **ไม่** ทำงานเฉพาะกรณีเดียว: บอทที่ล็อกไว้ก่อนมีฟีเจอร์นี้ (`lockedLineDisplayName` เป็น `null` — ไม่เคยบันทึกชื่อไว้เลย) จะไม่ถูกลงโทษด้วยชื่อที่ไม่เคยบันทึกไว้ ดูหัวข้อ "การอัปเกรดบอทเก่า" ด้านล่างว่าบอทกลุ่มนี้ถูกจัดการยังไง — **แต่ถ้าเคยบันทึกชื่อไว้แล้ว (`lockedLineDisplayName` ไม่ใช่ null) การ login ครั้งถัดไปที่ LINE ส่งชื่อว่างกลับมาก็ถือเป็น `name_mismatch` เช่นกัน ไม่มีข้อยกเว้นให้ฝั่งที่ส่งเข้ามาว่างอีกต่อไป** (แก้แล้วเมื่อ 21 สิงหาคม 2026 — เดิมเป็นช่องให้บัญชีที่ถูกแชร์/โอนต่อสามารถ login ผ่านได้เงียบๆ ถ้า client ฝั่งนั้นบังเอิญไม่ส่ง displayName กลับมา แลกกับความเสี่ยงที่ LINE ส่งชื่อว่างมาชั่วคราวจริงๆ ก็จะโดนล็อกเอาท์ด้วยเช่นกัน ต้องใช้ "รีเซ็ตล็อกบัญชี" เพื่อกู้คืน)
...
| `backend/src/api/forwarded-events.ts` | รายชื่อ event ที่ส่งไป dashboard ผ่าน `/ws` รวม `id_lock_mismatch` (มี field `reason`) |
