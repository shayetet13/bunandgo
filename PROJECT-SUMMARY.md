...
| **Server 2** | Linode / Akamai Connected Cloud (Tokyo)              | `10.77.0.2` (private, เข้าผ่าน ProxyJump ทาง Server 1) | **Backend จริง** — รัน bot ทุกตัว, process เดียว (`linebot-worker`, port 8791) ดูแลทุก owner                     |
...
4. **Hot/discard ceiling** — lane ที่วัดได้ช้ากว่า `applicationHotCeilingMs` (18ms) เริ่มไม่ถูกเลือกเป็นอันดับแรก, ช้ากว่า `applicationDiscardCeilingMs` (20ms) ถูกตัดเข้า repair — ค่าพวกนี้ตั้งผ่าน env บน production ไม่ใช่ default ใน source code (ต้อง SSH เช็คสดเสมอ ห้ามเชื่อ source)
