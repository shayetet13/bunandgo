...
Server 2 runs three permanent runtime processes against one shared SQLite
database. Primary is the public control plane on port 8791; Shard B and Shard
C are private, on ports 8792 and 8793. An owner and all of that owner's bots
always run in exactly one process.
...
free core, not a bigger lane pool on one process. Add Shard C (or a further
Shard D, following the same pattern) whenever a shard's own CPU/event-loop
watchdog samples run hot, or whenever total zero-delay poll demand
(`fastPollSlots` summed across shards) is smaller than the number of bots
that genuinely need the sub-100ms tier — a bot that is fine with the default
100ms fast-poll interval never consumes a slot at all, so route only the
latency-sensitive ones onto the zero-delay tier.
...
| Property              | Primary                           | Shard B                             | Shard C                            |
| ---------------------- | ---------------------------------- | ------------------------------------ | ------------------------------------ |
| Worker ID             | `primary`                         | `shard-b`                           | `shard-c`                          |
| API port              | 8791                               | 8792, loopback only                 | 8793, loopback only                |
| LINE lane source      | 16 local                           | 16 local                             | 16 local                            |
| Send-reserved lanes   | 4                                   | 4                                     | 4                                    |
| Zero-delay poll slots | 8                                   | 8                                     | 8                                    |
| Owner allocation      | fewest owners, ties land here first | fewest owners, ties land here second | fewest owners, ties land here third |
...
`WORKER_ASSIGNMENT_WORKERS` (Primary, then Shard B, then Shard C — this order
is derived automatically from the `primary`/`shards` order in
`worker-topology.json`). Later bot creation, login, reconnect, restart, and
deploy all reuse that row — **existing owners are never rebalanced
automatically**, so adding Shard C only changes where *new* owners land. To
spread bots that were assigned before Shard C existed (e.g. to move some of
15+ existing bots off Primary/Shard B), update the affected rows in
`owner_worker_assignments` by hand and restart each of those bots so it
reconnects on its newly assigned worker. Deleting the user removes its
obsolete assignment.

Primary routes requests for Shard B and Shard C owners over loopback. Each
shard accepts only authenticated forwards from Primary and reports events
back to Primary. All three services share `CONTROL_PLANE_TOKEN`, but the
token lives only in the protected runtime topology. Static
`WORKER_OWNER_SCOPE`, `WORKER_OWNER_EXCLUDE`, and
`WORKER_OWNER_ROUTES` values remain empty.

## Lane and latency policy

Primary, Shard B, and Shard C each own 16 process-local H2 lanes. Four
...
| Shard C | `linebot-worker-shard-c` | `/etc/linebot/worker-shard-c.env` |
...
workers. Ports 8791, 8792, and 8793 must remain private; Server 1 is the only
TLS edge.
...
Bringing Shard C online for the first time: install `worker-shard-c.env` from
its example, add the `shard-c` entry to the shared `worker-topology.json`
(already shown above), then create `/etc/linebot/worker-shard-c-enabled`
before starting `linebot-worker-shard-c.service`. Restart Primary and Shard B
too, since they re-read the same topology file at boot and need to learn
about the new peer.

## Inspection

```bash
journalctl -u linebot-worker -u linebot-worker-shard-b -u linebot-worker-shard-c \
...
The control-plane metrics endpoint is the authoritative combined view of all
three local worker processes.
