...
rem Snapshot only operational keys. Every secret is intentionally absent. The
rem journal stream itself is not filtered, preserving exact ordering.
set "REMOTE_CMD=set -eu; echo '=== watcher snapshot begin ==='; date -Is; for unit in linebot-worker; do state=$(systemctl is-active $unit 2>/dev/null || true); pid=$(systemctl show -p MainPID --value $unit 2>/dev/null || echo 0); echo SERVICE unit=$unit active=${state:-unknown} MainPID=${pid:-0}; if [ ${pid:-0} -gt 0 ] && [ -r /proc/$pid/environ ]; then tr '\0' '\n' < /proc/$pid/environ | grep -E '^(WORKER_ID|PORT|DISPATCH_ADDR|LINE_H2_LANES|LINE_EFFECTIVE_H2_LANES|SQUARE_FAST_POLL_INTERVAL_MS|SQUARE_FAST_POLL_SLOTS)=' | sort || true; fi; done; echo '--- bot inventory: id|name|owner_user_id|status ---'; if command -v sqlite3 >/dev/null 2>&1; then sqlite3 -header -separator '|' /opt/linebot/shared/worker.db 'SELECT id, replace(replace(name,char(10),char(32)),char(13),char(32)) AS name, COALESCE(CAST(owner_user_id AS TEXT),char(45)) AS owner_user_id, status FROM bots ORDER BY id;' || echo '[snapshot] bot inventory query failed'; else echo '[snapshot] sqlite3 is unavailable'; fi; echo '=== watcher snapshot end; raw journal follows ==='; exec journalctl -u linebot-worker -n 200 -f --no-hostname -o short-iso-precise"
...
echo [watch-log-bigsa.bat] restart, and system-load evidence is not discarded.
echo [watch-log-bigsa.bat] Local raw copy: "%LOG_FILE%"
echo [watch-log-bigsa.bat] Snapshot includes service/PID, non-secret effective settings, and all bots.
